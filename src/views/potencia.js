...
export const Potencia = props => {
    const [resistenciaVal, setResistenciaVal] = useState({result: ""});
    const [potenVal, setPotenVal] = useState({result: ""});
    const handleSubmit = (a,b) => {
        setResistenciaVal({result: `Potencia: ${parseFloat(a*b).toFixed(4)}W`});
        setPotenVal({result: `Resistencia: ${parseFloat(a/b).toFixed(4)}W`})
    }
    return (
        <>
