...
export const Tester = props => {
    const [voltageVal, setVoltageVal] = useState({result: ""});
    const [potenVal, setPotenVal] = useState({result: ""});
    const handleSubmit = (a,b) => {
        setVoltageVal({result: `Voltage: ${parseFloat(a*b).toFixed(4)}V`});
        setPotenVal({result: `Potencia: ${parseFloat((a*b)*a).toFixed(4)}W`})
    }
    return (
        <>
           <div className="container py-4">
                <div className="card border-warning">
                    <div className="card-header text-center text-white py-5 border-warning bodySvg">
                        <h2>Voltage - Potencia</h2>
                    </div>
                    <div className="card-body py-5 bodySvg">
                            <form className="row" onSubmit={(event)=>event.preventDefault()}>
                                <Event onSubmit={handleSubmit} txtValue1="Intensidad" txtValue2="Resistencia"/>
                            </form>
                            <div className="mt-3">
                                <p className="text-light">{voltageVal.result}</p>
